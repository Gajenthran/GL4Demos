...
static void initGL(void) {
  ...
  _pId  = gl4duCreateProgram("<vs>shaders/stars.vs", "<fs>shaders/stars.fs", NULL);
...
static void sphereMove(void) {
  static float lf = FREQ_MOY;

  if(lf < FREQ_MOY && _basses > FREQ_MOY) {
    if(_state == 0) {
      _mode = 0;
      _state++;
      return;
